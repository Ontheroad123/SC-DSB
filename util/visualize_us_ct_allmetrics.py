...
class InferenceResultVisualizer(BaseVisualizer):
    ...
    def draw(self, epoch, iters, xs, labels, inputs,  xrange=(-1, 1), yrange=(-1, 1), subfix=None):
        ...
        if self.args.exp2d:
            ...
            metrics = compute_metrics(samples, labels)
            mse_loss = self.criterion(samples, labels).mean(dim=-1)
            
            raw_loss = mse_loss
            loss = raw_loss.mean()
            metrics['loss_v'] = loss.detach().cpu().numpy()
            save_image(samples, (os.path.join(save_path, f'ep{epoch}_it{iters}.png')), nrow=4, normalize=True, value_range=(-1, 1))
            save_image(labels, (os.path.join(save_path, f'ep{epoch}_it{iters}_l.png')), nrow=4, normalize=True, value_range=(-1, 1))
            save_image(inputs, (os.path.join(save_path, f'ep{epoch}_it{iters}_i.png')), nrow=4, normalize=True, value_range=(-1, 1))
...
class TrajectoryVisualizer(BaseVisualizer):
    ...
    def draw_img(self, epoch, iters, xs, subfix=None):
        ...
        for i in range(xs.shape[1]):
            _xs = xs[:, i]
            save_image(_xs, (os.path.join(save_path, f'{i}.png')), nrow=12, normalize=True, value_range=(-1, 1))
            with torch.cuda.device('cuda'):
                torch.cuda.empty_cache()
