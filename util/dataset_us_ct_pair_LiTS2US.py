...
def _list_image_files_recursively(data_dir):
    ...
    print(data_dir)
    if 'val' in data_dir:
        filname = './dataset/LiTS-US/ct-us-val.txt'
        ...
        filname = './dataset/LiTS-US/ct-us.txt'
        with open(filname, 'r') as fp:
            infos = fp.readlines()
            for info in infos:
                tmp = info.split(',')
...
class ImageAFHQDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        #print(self.imgs[idx % len(self.imgs)])
...
if __name__ == '__main__':
    ...
    dataset, _, dataloader = create_data('afhq-val-dog-384')
