...
class Downsample2D(nn.Module):
    ...
    def forward(self, x, k):
        ...
        Args:
            ...
            k: 下采样因子，例如 2 表示将图像高度和宽度缩小一半

        Returns:
            下采样后的张量，形状为 (batch_size, channels, height//k, width//k)
        """
        # 使用平均池化进行下采样
        return nn.AvgPool2d(kernel_size=k, stride=k)(x)
...
class Discriminator_large(nn.Module):
  ...
  def forward(self, x_tm1, x_t, t):
    #print(up_or_down_sampling.downsample_2d)
